...
/**
 * Class Wp_Login_Logo_Operation
 * @package ChangeMyAdminLoginLogo\Includes
 */
class  Wp_Login_Logo_Operation {

	public function cmywll_change_my_login_logo_operation() {
		$message = "";
		...
		if ( $bool ) {
			...
			$url_option = ( get_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_URL, null ) !== null );

			if ( ! is_numeric( $width ) || ! is_numeric( $height ) ) {
				$message = self::cmywll_change_my_login_logo_validation_message( "alert alert-danger", "Invalid Height/ Width value." );
			}
			if ( empty( trim( $location ) ) ) {
				$message = self::cmywll_change_my_login_logo_validation_message( "alert alert-danger", "Invalid image file location." );
			}

			if ( empty( $message ) ) {
				if ( $url_option ) {
					update_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_URL, $location );
				} else {
					add_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_URL, $location );
				}
				$height_option = ( get_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_HEIGHT, null ) !== null );
				if ( $height_option ) {
					update_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_HEIGHT, $height );
				} else {
					add_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_HEIGHT, $height );
				}
				$width_option = ( get_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_WIDTH, null ) !== null );
				if ( $width_option ) {
					update_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_WIDTH, $width );
				} else {
					add_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_WIDTH, $width );
				}
				$message = self::cmywll_change_my_login_logo_validation_message( "alert alert-success", "Data saved successfully" );
			}
		}
		$location = esc_url( get_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_URL ) );
		$width    = get_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_WIDTH );
		$height   = get_option( Option_Constants::CHANGE_MY_LOGIN_LOGO_HEIGHT );
	...
	private function cmywll_change_my_login_logo_validation_message( $class_name, $message ) {
